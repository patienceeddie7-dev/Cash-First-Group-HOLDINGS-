...
export const CustomerPortal: React.FC = () => {
  ...
  // Compulsory Fields State
  const [clientPhotoUrl, setClientPhotoUrl] = useState<string>(activeCustomer?.avatarUrl || activeCustomer?.selfieUrl || 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200&auto=format&fit=crop&q=80');
  const [collateralName, setCollateralName] = useState<string>('Toyota Vitz Reg MW-1092');
  const [collateralPhotoUrl, setCollateralPhotoUrl] = useState<string>('https://images.unsplash.com/photo-1549399542-7e3f8b79c341?w=400&auto=format&fit=crop&q=80');
  ...
  const handleApplySubmit = (e: React.FormEvent) => {
    ...
    if (!clientPhotoUrl) {
      alert('COMPULSORY REQUIREMENT MISSING:\n\nPlease upload or capture your Client/Borrower Photo before submitting.');
      return;
    }
    if (!collateralName.trim()) {
      alert('COMPULSORY REQUIREMENT MISSING:\n\nPlease enter the Name and Description of your Collateral Item.');
      return;
    }
    if (!collateralPhotoUrl) {
      alert('COMPULSORY REQUIREMENT MISSING:\n\nPlease upload a Photo of your Collateral Item.');
      return;
    }
    if (!signatureData) {
      alert('Please provide your electronic signature before submitting the agreement.');
      return;
    }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="space-y-6">
          ...
          <form onSubmit={handleApplySubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-5 text-xs">
              ...
              {/* COMPULSORY CLIENT PHOTO & COLLATERAL SECTION */}
              <div className="bg-rose-50/50 border-2 border-rose-200 rounded-xl p-4 space-y-4">
                <div className="flex items-center justify-between border-b border-rose-200/80 pb-2">
                  <div className="flex items-center gap-2">
                    <AlertCircle className="w-4 h-4 text-rose-600" />
                    <span className="font-bold text-slate-900 text-xs">Compulsory Verification Requirements</span>
                  </div>
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 mb-2">Upload a clear passport photo or selfie for identity verification.</p>
                  
                  <div className="flex items-start gap-3">
                    <div className="w-16 h-16 rounded-lg bg-slate-200 border border-slate-300 overflow-hidden flex-shrink-0 flex items-center justify-center">
                      {clientPhotoUrl ? (
                    ...
                    <div className="flex-1 space-y-1.5">
                      <label className="cursor-pointer bg-white border border-slate-300 hover:border-teal-600 text-slate-700 hover:text-teal-700 px-3 py-1.5 rounded-lg text-xs font-bold inline-flex items-center gap-1.5 shadow-sm transition-all">
                        <Upload className="w-3.5 h-3.5 text-teal-600" /> Upload Client Photo
                        <input
                          type="file"
                          accept="image/*"
                          className="hidden"
                      ...
                      <div className="text-[10px] text-slate-500 flex items-center gap-1">
                        {clientPhotoUrl ? (
                          <span className="text-emerald-700 font-bold flex items-center gap-1">
                            <CheckCircle className="w-3 h-3" /> Client photo uploaded
                          </span>
                        ) : (
                          <span className="text-rose-600 font-semibold">Photo required</span>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-300 rounded-lg text-xs font-semibold bg-white text-slate-800 focus:ring-2 focus:ring-teal-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 mb-2">Upload a clear photo showing the collateral item condition & serial/registration.</p>

                  <div className="flex items-start gap-3">
                    <div className="w-24 h-16 rounded-lg bg-slate-200 border border-slate-300 overflow-hidden flex-shrink-0 flex items-center justify-center">
                      {collateralPhotoUrl ? (
                        <img src={collateralPhotoUrl} alt="Collateral Photo" className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-[10px] text-slate-400 font-semibold text-center">No Photo</span>
                      )}
                    </div>
                    <div className="flex-1 space-y-2">
                      <label className="cursor-pointer bg-white border border-slate-300 hover:border-teal-600 text-slate-700 hover:text-teal-700 px-3 py-1.5 rounded-lg text-xs font-bold inline-flex items-center gap-1.5 shadow-sm transition-all">
                        <Upload className="w-3.5 h-3.5 text-teal-600" /> Upload Collateral Photo
                        <input
                          type="file"
                          accept="image/*"
                          className="hidden"
                      ...
                      <div className="flex flex-wrap gap-1 text-[10px]">
                        <span className="text-slate-400 font-semibold self-center mr-1">Quick Samples:</span>
                        <button
                          type="button"
                          onClick={() => {
                            setCollateralName('Toyota Vitz Reg MW-1092');
                            setCollateralPhotoUrl('https://images.unsplash.com/photo-1549399542-7e3f8b79c341?w=400&auto=format&fit=crop&q=80');
                          }}
                          className="bg-white hover:bg-slate-100 border border-slate-200 px-2 py-0.5 rounded text-slate-600 font-medium"
                        >
                          🚗 Vehicle
                        </button>
                        <button
                          ...
                          onClick={() => {
                            ...
                            setCollateralPhotoUrl('https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=400&auto=format&fit=crop&q=80');
                          }}
                          className="bg-white hover:bg-slate-100 border border-slate-200 px-2 py-0.5 rounded text-slate-600 font-medium"
                        >
                          🏡 Land/Deed
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setCollateralPhotoUrl('https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=400&auto=format&fit=crop&q=80');
                          }}
                          className="bg-white hover:bg-slate-100 border border-slate-200 px-2 py-0.5 rounded text-slate-600 font-medium"
                        >
                          💻 Laptop/Tech
                      ...
                      <div className="text-[10px]">
                        {collateralPhotoUrl ? (
                          <span className="text-emerald-700 font-bold flex items-center gap-1">
                            <CheckCircle className="w-3 h-3" /> Collateral photo uploaded
                          </span>
                        ) : (
                          <span className="text-rose-600 font-semibold">Collateral photo required</span>
                        )}
                      </div>
                    </div>
                  </div>
                          ...
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-full overflow-hidden bg-slate-200 border border-slate-300 flex-shrink-0">
                              {app.clientPhotoUrl ? (
                                ...
                                <User className="w-5 h-5 text-slate-400 m-1" />
                              )}
                            </div>
      ...
      {activeTab === 'kyc' && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* National ID */}
            <div className="border border-slate-200 rounded-xl p-4 text-xs space-y-3 bg-slate-50/50">
              <span className="font-bold text-slate-800 block">1. National ID / Passport</span>
              {activeCustomer.nationalIdDocUrl ? (
                <img src={activeCustomer.nationalIdDocUrl} alt="ID Document" className="w-full h-32 object-cover rounded-lg border border-slate-300" />
              ) : (
                <div className="h-32 border-2 border-dashed border-slate-300 rounded-lg flex flex-col items-center justify-center text-slate-400 p-2 text-center">
                  <Upload className="w-6 h-6 mb-1" /> Click or drag National ID photo here
                </div>
              )}
              <button
                type="button"
                onClick={() => updateCustomerKYC(activeCustomer.id, 'Pending', { nationalIdDocUrl: 'https://images.unsplash.com/photo-1586281380349-632531db7ed4?w=400&auto=format&fit=crop&q=80' })}
                className="w-full bg-slate-200 hover:bg-slate-300 text-slate-800 font-semibold py-2 rounded-lg text-xs"
              >
                Upload ID Document
              </button>
            </div>
            ...
            {/* Selfie Photo */}
            <div className="border border-slate-200 rounded-xl p-4 text-xs space-y-3 bg-slate-50/50">
              <span className="font-bold text-slate-800 block">2. Facial Selfie Photo</span>
              {activeCustomer.selfieUrl ? (
                <img src={activeCustomer.selfieUrl} alt="Selfie" className="w-full h-32 object-cover rounded-lg border border-slate-300" />
              ) : (
                <div className="h-32 border-2 border-dashed border-slate-300 rounded-lg flex flex-col items-center justify-center text-slate-400 p-2 text-center">
                  <User className="w-6 h-6 mb-1" /> Take or upload selfie photo
                </div>
              )}
              <button
                type="button"
                onClick={() => updateCustomerKYC(activeCustomer.id, 'Pending', { selfieUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80' })}
                className="w-full bg-slate-200 hover:bg-slate-300 text-slate-800 font-semibold py-2 rounded-lg text-xs"
              >
                Upload Selfie Photo
              </button>
            ...
            {/* Income Proof */}
            <div className="border border-slate-200 rounded-xl p-4 text-xs space-y-3 bg-slate-50/50">
              <span className="font-bold text-slate-800 block">3. Proof of Income / Bank Statement</span>
              {activeCustomer.incomeProofUrl ? (
                <img src={activeCustomer.incomeProofUrl} alt="Income proof" className="w-full h-32 object-cover rounded-lg border border-slate-300" />
              ) : (
                <div className="h-32 border-2 border-dashed border-slate-300 rounded-lg flex flex-col items-center justify-center text-slate-400 p-2 text-center">
                  <FileText className="w-6 h-6 mb-1" /> Upload pay stub or bank statement
                </div>
              )}
              <button
                type="button"
                onClick={() => updateCustomerKYC(activeCustomer.id, 'Pending', { incomeProofUrl: 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=400&auto=format&fit=crop&q=80' })}
                className="w-full bg-slate-200 hover:bg-slate-300 text-slate-800 font-semibold py-2 rounded-lg text-xs"
              >
