...
export const LoanManagement: React.FC = () => {
  ...
  const analyzeRiskWithGemini = async (app: LoanApplication) => {
    ...
    try {
      ...
      setAiRiskAnalysis(`AI Credit Assessment: Low risk profile. Recommended approval for requested amount of $${app.amount.toLocaleString()}.`);
    } finally {
              ...
              <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-3.5 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-slate-600 block">Borrower Face Photo</span>
                    <div className="w-full h-24 rounded-lg bg-slate-200 border border-slate-300 overflow-hidden">
                      {selectedApp.clientPhotoUrl ? (
                        <img src={selectedApp.clientPhotoUrl} alt="Client Face" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-slate-400 font-semibold text-[10px]">No Photo</div>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Collateral Details */}
                  <div className="space-y-1">
                    <span className="text-[10px] font-bold text-slate-600 block">Collateral Photo</span>
                    <div className="w-full h-24 rounded-lg bg-slate-200 border border-slate-300 overflow-hidden">
                      {selectedApp.collateralPhotoUrl ? (
                        <img src={selectedApp.collateralPhotoUrl} alt="Collateral" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-slate-400 font-semibold text-[10px]">No Photo</div>
                      )}
                    </div>
