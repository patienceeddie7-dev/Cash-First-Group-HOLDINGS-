  ...
  useEffect(() => {
    try {
      const unsub = onSnapshot(
        ...
        (snapshot) => {
          const list: Customer[] = snapshot.docs.map((d) => d.data() as Customer);
          setCustomers(list);
  ...
  useEffect(() => {
    try {
      const unsub = onSnapshot(
        collection(db, 'staff'),
        (snapshot) => {
          const list: StaffUser[] = snapshot.docs.map((d) => d.data() as StaffUser);
          setStaffUsers(list);
        },
  ...
  useEffect(() => {
    try {
      const unsub = onSnapshot(
        ...
        (snapshot) => {
          const list: LoanApplication[] = snapshot.docs.map((d) => d.data() as LoanApplication);
